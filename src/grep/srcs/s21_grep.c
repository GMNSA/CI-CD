...
int read_patterns_from_file(s_data *data_, char const *filename_,
  ...
  if (!fin) {
    ...
    error_->p_error_name = s21_strdup(filename_);
  } else {
    while ((p_str = s21_fgets(fin, 1)) && *p_str != EOF) {
...
char *s21_strdup_char(const char ch_) {
  char *str = NULL;
  ...
  for (int i = 1; i < argc_ && error_->error == ERROR_OK; ++i) {
    if (is_read_from_file) {
      ...
      if (p_data->is_pattern || (is_pattern_found && i != 1)) {
        deep2_push(&p_patterns, s21_strdup(argv_[i]));
        is_pattern_found = 0;
        if (argc_ - 1 > i) {
          if (argv_[i + 1][0] != '-') {
            p_data->is_pattern = 0;
        ...
        deep2_push(&p_filenames, s21_strdup(argv_[i]));
      }
    } else {
      if (strlen(argv_[i]) > 2) {
        ...
        for (int j = 1; j < n_tmp_argv_ && error_->error == ERROR_OK; ++j) {
          ...
          if (argv_[i][j] == 'e') {
            ...
            if ((argv_[i][j + 1]) != '\0') {
              deep2_push(&p_patterns, s21_strdup(argv_[i] + j + 1));
              p_data->is_pattern = 0;
              is_pattern_found = 0;
              break;
            } else {
            ...
            if (!option_assignment(p_data, tmp)) {
              ...
              error_->p_error_name = s21_strdup_char(tmp[1]);
            }
            p_data->is_pattern = 1;
          }
        }
        ...
        if (!option_assignment(p_data, argv_[i])) {
          error_->error = ERROR_INVALID_OPTION;
          error_->p_error_name =
              s21_strdup_char(argv_[i][strlen(argv_[i] - 1)]);
          exit(-1);
        }
      }
    }
  ...
  for (unsigned j = 0; j < data->patterns.n_data; ++j) {
    ...
    if (line->result) {
      if (line->result == REG_ESPACE) {
        ...
        error_->p_error_name = s21_strdup("ERROR REGEX\n");
      }
    } else {
...
void s21_grep(int argc, char **argv) {
  ...
  if (argc < 3) {
    ...
    error.p_error_name = s21_strdup("grep: Few arguments.\n");
    ...
    for (unsigned i = 0; i < p_filenames->n_data; ++i) {
      ...
      if (fin) {
        line.n_line = 1;
        while ((line.p_str = s21_fgets(fin, 1)) && *line.p_str != EOF) {
          if (!(data.p_array_num =
        ...
        error.p_error_name = s21_strdup(p_filenames->data[i]);
